...
$(1-\beta)$를 검정력(power)라고 부른다. 즉 대립 가설(alternative hypothesis)이 사실일 때 이를 사실로 예측할 확률이다. negative로 판정한 것 중 true negative의 비율을 의미한다. 사실 NHST는 상당히 높은 수준의 검정력을 암묵적으로 전제한다.  하지만 이 전제가 ㅌ,ㄹㄹ 혹은 의도적으로 침묵한다면 어떤 일이 생길까?  
...
2. 현상이 TRUE인 경우가 몹시 드물다고 해보자. 99%가 FALSE이고 1% 정도만 TRUE라면? 이 경우 $\beta = 0.6$, $\alpha = 0.05$를 가정할 경우 FPRP는 무려 0.93이 된다. 즉, 이오니디스 선생의 주장대로 대부분의 연구가 가짜 연구가 된다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTc1NzY0MjU3NiwxMzczNjc3MzEzLC05Nj
UzNTE1OCwtMTA1NjI0NDgyMSwtMTYzOTQ1ODYwOF19
-->