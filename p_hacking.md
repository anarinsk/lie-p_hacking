...
앞서의 표를 살짝 다르게 표현해보자. positive, negative는 TR 각 예측은 맞거나 틀리거나 둘 중 하나에 속한다. 표에서 예측이 맞는 경우의 비율은 $(1-\alpha)$, $(1-\beta)$이고 그렇지 않은 비율은 $\alpha$, $\beta$다. 여기서할 것이다. 이를 비율로 다시 표현하자. 이때,  false pPositive 의 비율을 $\alpha$라고 하자. 이를 통계학에서는 1종 오류라고 한다. 반대로 false nNegative의 비율 $\beta$를 2종 오류라고 부른다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNzI5MTIxNzk0LC0xODY4Mjk4NzAwLDIxMT
